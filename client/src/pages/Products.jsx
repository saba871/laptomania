...
const Laptop = ({ laptop }) => {
  ...
  const media = (() => {
    ...
    if (typeof laptop.image === "string" && laptop.image) return [laptop.image];
    if (typeof laptop.images === "string" && laptop.images) return [laptop.images];
    return [];
  })();
    ...
    return (
      <article className="surface-card rounded-3xl border border-white/5 shadow-xl shadow-black/5 p-6">
        <form onSubmit={handleUpdate} className="flex flex-col gap-4">
          <h3 className="text-xl font-semibold">Editing: {laptop.brand} {laptop.model}</h3>
          {editableFields.map((key) => (
            <div key={key} className="flex flex-col gap-1">
              <label className="eyebrow-label">{formatLabel(key)}</label>
    ...
    <article className="surface-card flex flex-col sm:flex-row rounded-3xl border border-white/5 shadow-xl shadow-black/5 transition duration-300 hover:-translate-y-1 hover:border-white/10 overflow-hidden">
        ...
        <figure>
          {mainImage ? (
            ...
            <div className="flex h-full w-full flex-col items-center justify-center text-center text-sm text-muted p-4 min-h-[250px]">
              <span className="text-lg font-semibold">{laptop.brand || "Unknown"}</span>
              <span>No photo provided</span>
            </div>
          )}
        </figure>
        <div className="mt-4 rounded-2xl border border-white/10 bg-black/20 px-4 py-3 text-center">
          <p className="text-xs uppercase tracking-[0.35em] text-white/70">Drop price</p>
          <p className="text-2xl font-semibold text-white">${laptop.price || 0}</p>
        </div>
      </div>
      ...
      <div className="flex flex-1 flex-col justify-between gap-4 p-6 sm:w-3/5">
        ...
        <div>
            ...
            <p className="eyebrow-label mb-3 text-muted">{laptop.series || laptop.category || "Laptop"}</p>

            {/* ძირითადი მახასიათებლები - უფრო სუფთა ცხრილის ფორმატი */}
            <div className="grid grid-cols-2 gap-x-4 gap-y-2 text-sm">
                <span className="text-white">Processor: <span className="text-muted">{laptop.processor || laptop.proccesor || "N/A"}</span></span>
                <span className="text-white">RAM: <span className="text-muted">{laptop.ram || "N/A"}</span></span>
                <span className="text-white">Storage: <span className="text-muted">{laptop.storage || "N/A"}</span></span>
                <span className="text-white">OS: <span className="text-muted">{laptop.os || "N/A"}</span></span>
            </div>
        </div>

        {/* აღწერა - უფრო კომპაქტური */}
        <p className="text-sm leading-relaxed text-muted mt-3 line-clamp-2">
          {laptop.description || "No description has been added for this laptop yet."}
        </p>

        {/* მოქმედებები და მარაგი */}
        <div className="flex flex-wrap items-center justify-between gap-3 pt-4 border-t border-white/10">
            <span className="text-xs uppercase tracking-[0.35em] text-muted">
                Stock: <strong className="tracking-normal text-white">{laptop.stock || 0}</strong> | Condition: {laptop.condition || "Available"}
            </span>
      ...
      <header className="surface-card" data-variant="subtle">
        <div className="flex flex-wrap items-end justify-between gap-6">
          <div className="space-y-3">
            ...
            <h1 className="text-4xl font-semibold tracking-tight">Laptops</h1>
            <p className="text-muted max-w-2xl">
              Hand-picked premium devices curated for performance and endurance.
            </p>
          </div>
          {user ? (
            ...
            <span className="text-xs uppercase tracking-[0.4em] text-muted">Browse only</span>
          )}
        </div>
      </header>
